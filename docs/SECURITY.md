...
- The digest MUST carry a warning when a tasks shard's **last commit came from
  ...
  *that* member, and only a difference is the warning
  ...
  else at `join` `[C bin/handshake.js:534,708,2115]` — and a peer's email is
...
- **One flag, and the mismatch takes precedence.** Both lists are always
  ...
  off it `[C lib/workspace-files.js:524-533]` `[C bin/handshake.js:1314-1319]`,
  ...
  `[C bin/handshake.js:1276-1281]`. The guarantee is therefore that a shard is
...
- **The member name is the one field that can carry your local username.**
  ...
  `[C bin/handshake.js:412-417]` `[C bin/handshake.js:455-456]`
  `[C bin/handshake.js:2060-2061]`. That derivation is lossier than
  ...
  `-` and the result truncated to 48 chars `[C bin/handshake.js:415]`. `Ann Lee`
  ...
  `[C bin/handshake.js:416]` — their own name erased rather than transliterated,
  ...
  `[C bin/handshake.js:644-646]`, so the one route that takes an arbitrary `--as`
...
- **Where that derived name travels depends on the tier.** On ntfy nobody
  ...
  `[C bin/handshake.js:463,489]` and it goes out as `from.member` in every
  envelope `[C bin/handshake.js:177-179,208]`, inside the signed canonical form
  ...
  `[C bin/handshake.js:470,489,2066]` `[C relay/src/do/workspace.js:603-604]`.
  ...
  `[C bin/handshake.js:455-456]` `[C bin/handshake.js:2060-2061]`. `--as <name>` on either command replaces it
  outright, and `join` never derives at all: it takes `--as`, or asks the human
  for a member name `[C bin/handshake.js:641-643]`. Pick the name you want peers
