...
// -------------------------------------------- the state-branch batch clock --
//
// V2-PLAN 10.1. Two tiny documents in the state dir, and neither is
// `state.json`: that file is read-modify-written by hooks on hot paths, and a
// batch clock that fights the claim writer for one JSON document is a lost
// claim (section 10.1's Touches says so in as many words).
//
// `state.beat` carries BOTH the clock and the outcome, and the two are
// deliberately different fields. `at` is the clock - when a batch last actually
// RAN - and it is what makes "two batches inside one minute produce one commit"
// true across the monitor and the Stop hook, which are separate processes with
// no shared memory. `outcome`/`push` are what the last attempt concluded, and
// an attempt that never started for want of time rewrites those WITHOUT moving
// `at`, so the next turn retries instead of waiting out a minute it did not
// spend. mtime is not the clock here, unlike `stop.beat`, for exactly that
// reason.
function readStateBeat(state) {
  const raw = readJsonFile(state.files.state_beat, null);
  // `Number(null)` is 0 and `Number('')` is 0, both finite - so a null clock
  // read through a bare `Number.isFinite` becomes the epoch, which reads as
  // "a batch ran in 1970" and holds the ≤ 1/min gate open forever. The type is
  // checked before the value, here and in the writer below.
  const num = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : null);
  const str = (v) => (typeof v === 'string' && v ? v : null);
  return {
    present: Boolean(raw),
    at: raw ? num(raw.at) : null,                 // the CLOCK: the last beat that ran
    reported_at: raw ? num(raw.reported_at) : null,
    outcome: raw ? str(raw.outcome) : null,
    push: raw ? str(raw.push) : null,
    reason: raw ? str(raw.reason) : null,
    detail: raw ? str(raw.detail) : null,
    commit: raw ? str(raw.commit) : null,
    where: raw ? str(raw.where) : null,           // monitor | stop | session_end | session_start | cli
    late_flush: Boolean(raw && raw.late_flush),
  };
}

// THE CLOCK FIELD HAS THREE INPUTS AND THEY MEAN THREE DIFFERENT THINGS, which
// is why it is not one expression:
//   absent   - "I am recording an outcome, leave the clock alone"
//   null     - "there is no clock: no batch has run, or the feature was just
//              switched off, so the next beat must run immediately"
//   a number - "a batch ran at this instant"
// Collapsing the first two loses the retry-now property the no-time arm needs
// [C monitors/heartbeat.js stateBatch]; collapsing null into a number is the
// `Number(null) === 0` trap the reader above documents.
function writeStateBeat(state, rec) {
  const r = rec || {};
  const prev = readStateBeat(state);
  let at;
  if (r.at === undefined) at = prev.at;
  else if (r.at === null) at = null;
  else at = typeof r.at === 'number' && Number.isFinite(r.at) ? r.at : prev.at;
  const doc = {
    v: 1,
    at,
    reported_at: Number.isFinite(Number(r.reported_at)) ? Number(r.reported_at) : Date.now(),
    outcome: typeof r.outcome === 'string' ? r.outcome : null,
    push: typeof r.push === 'string' ? r.push : null,
    ...
    late_flush: r.late_flush === true,
  };
  ensureDir(state.dir);
  writeJsonFile(state.files.state_beat, doc);
  return doc;
}

// `state.pending` is the late-not-lost marker: a shard record is on disk and
// its batch has not reached the remote. SessionEnd's flush is best-effort by
// contract, 20 of 21 [S4], so the one session in twenty-one that loses it
// leaves this behind and the NEXT session start commits the batch - late
// instead of lost. Presence is the whole signal; the body is advisory.
function markStatePending(state, why) {
  try {
    ensureDir(state.dir);
    fs.writeFileSync(state.files.state_pending,
      JSON.stringify({ at: Date.now(), why: String(why || 'shard_write').slice(0, 60) }) + '\n',
      { mode: FILE_MODE });
    return true;
...
function clearStatePending(state) {
  try { fs.unlinkSync(state.files.state_pending); return true; } catch (_) { return false; }
}

function statePending(state) {
  try { return fs.statSync(state.files.state_pending).mtimeMs; } catch (_) { return null; }
}
...
class State {
  constructor(ws, opts) {
    ...
    this.files = {
      ...
      // V2-PLAN 10.1's Touches: the deferred-push marker and the last-pushed
      // head record are sentinels beside the existing ones and are explicitly
      // NOT in state.json, which hooks read-modify-write on hot paths. They are
      // named here so the four writers (lib/state-branch.js, the monitor, the
      // Stop hook and the CLI) share one spelling of each path.
      state_branch_optin: path.join(this.dir, 'state-branch.optin.json'),
      state_branch_heads: path.join(this.dir, 'state-branch.heads.json'),
      state_beat: path.join(this.dir, 'state.beat'),
      state_pending: path.join(this.dir, 'state.pending'),
    };
...
module.exports = {
  ...
  readStateBeat, writeStateBeat, markStatePending, clearStatePending, statePending,
