...
const TAG_RES = [
  ...
  // Bounded on BOTH sides: `<{2,}` / `>{2,}` were unbounded, which is quadratic
  // on a bracket flood — a 125 KB attacker-committed shard took 18.5 s to read
  // (32 k brackets = 6 s). The real marker is exactly 3 brackets; 8 is already
  // far past any legitimate shape.
  /<{2,8}\s*\/?\s*handshake\s*:?[a-z0-9_-]{0,40}\s*>{2,8}/gi,
