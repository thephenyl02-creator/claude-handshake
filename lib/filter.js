...
const PATTERNS = [
  ...
  // No \b, for the reason stated at the top of this table: glued to a word
  // char these leaked 82-97% (Twilio SK 97.2%, Shopify 95.6%, DO 82.0%).
  { id: 'branded-token',     re: /(SK[0-9a-f]{32}|shpat_[a-f0-9]{32}|hvs\.[A-Za-z0-9_-]{20,}|dop_v1_[a-f0-9]{64}|rk_(live|test)_[A-Za-z0-9]{16,}|sk-proj-[A-Za-z0-9_-]{20,})/ },
...
// sources.
const SECRET_FILE_RE = /^(?:\.env(?:\..+)?|.*\.(?:pem|key|p12|pfx)|id_[a-z0-9]+|credentials(?:\.[a-z]+)?|secrets?\.(?:json|ya?ml|toml|ini)|\.npmrc|\.netrc|\.pgpass|service-account.*\.json|.*[_-]secrets?\.(?:json|ya?ml|toml|ini)|database\.ya?ml|application(?:-\w+)?\.(?:ya?ml|properties)|settings\.ya?ml)$/i;
...
// WHAT THE 12-CHARACTER WINDOW MAY SLIDE ALONG, and `null` for a needle it may
// not. The window exists for a CHUNKED leak - half a key pasted into a note -
// and it collides with ordinary English when it is pointed at prose, because a
// secret-bearing file carries prose beside its secrets and `readSecretValues`
// walks every string in it.
//
// Whitespace is the cheap test for prose and it is very nearly right, but not
// quite: `SECRET_FILE_RE` names `service-account*.json` explicitly, and the
// `private_key` value inside one is the canonical multi-line
// `-----BEGIN PRIVATE KEY-----\nMIIE...\n-----END PRIVATE KEY-----` string. A
// whitespace test alone drops the window for exactly that file - the shape most
// likely to be pasted BY THE CHUNK, one 64-character base64 line at a time -
// and leaves only the whole-string exact match, which such a paste never trips.
// (The dedicated PEM arm of `readSecretValues` covers `.pem`/`.key` files; the
// JSON arm walks string values verbatim and does not.)
//
// So the test is SHAPE, not whitespace: strip PEM armour, split on whitespace,
// and take the needle only when every piece is base64 alphabet and every piece
// but the last is at least 32 characters - which is how wrapped credential
// material looks and is nothing like how a sentence looks. English words are
// short and carry punctuation, so the measured false positive
// ("CREDENTIAL FILE. Workspace secret ...") is still prose and still gets
// exact-match-only.
const B64_RUN = 32;
function windowNeedle(raw) {
  if (!/\s/.test(raw)) return raw;                        // one token: the common case
  const bare = raw.replace(/-----[^-]+-----/g, ' ');
  const chunks = bare.split(/\s+/).filter(Boolean);
  if (!chunks.length) return null;
  if (!chunks.every((c) => /^[A-Za-z0-9+/=]+$/.test(c))) return null;
  if (!chunks.slice(0, -1).every((c) => c.length >= B64_RUN)) return null;
  const packed = chunks.join('');
  return packed.length > TRIPWIRE_WINDOW ? packed : null;
}
...
function tripwireFindings(text, opts) {
  ...
  for (const raw of readSecretValues(files)) {
    ...
    // THE PARTIAL (12-char window) MATCH IS FOR CREDENTIAL MATERIAL ONLY, and
    // the test for that is whitespace. A secret-bearing file carries prose as
    // well as secrets - this project's own `.handshake/secret.json` has a
    // `warning` sentence beside the workspace secret, and `readSecretValues`
    // walks EVERY string in the JSON - and sliding a 12-character window along
    // an English sentence collides with ordinary English. Measured: the needle
    // "CREDENTIAL FILE. Workspace secret ... (SECURITY.md 3.1)." blocked the
    // ordinary text "surfaced as a non-member commit warning (SECURITY.md
    // 5.4)." as a local-secret leak, on the shipped outbound path.
    // What is NOT weakened: the EXACT substring match above still runs against
    // every needle including a whitespace passphrase, and a needle that is
    // whitespace-BEARING credential material - a wrapped PEM body, a JSON
    // `private_key` - keeps the window on its packed form (see `windowNeedle`).
    // `haystacks` already carries a whitespace-stripped copy of the text
    // [C lib/filter.js variantsOf], so a packed needle meets a packed haystack.
    const needle = hit ? null : windowNeedle(raw);
    if (!hit && needle && needle.length > TRIPWIRE_WINDOW) {
      const packed = needle.toLowerCase();
      for (let i = 0; i + TRIPWIRE_WINDOW <= packed.length && !hit; i += 4) {
        const w = packed.slice(i, i + TRIPWIRE_WINDOW);
...
// `check` and its contract are UNCHANGED, and so is MAX_BYTES. The additions
// below exist for a SECOND caller - V2-PLAN 4.2 item 1's commit scanner in
// lib/state-branch.js, which calls `check()` PER RECORD (a shard outgrows
// MAX_BYTES on day one; a record does not) and hoists the tripwire corpus once
// per scan through `defaultSecretFiles` so a beat does not walk the project
// every minute. It is the same battery on the same contract, deliberately: a
// second battery beside this one is a second answer to "may these bytes leave
// the machine", and a second answer drifts.
module.exports = {
  check, MAX_BYTES, PATTERNS, tripwireFindings, defaultSecretFiles, SECRET_FILE_RE,
  _internals: { shannon, variantsOf, readSecretValues },
};
