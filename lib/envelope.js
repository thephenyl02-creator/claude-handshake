...
// Walk EVERY string in the body structurally, so a field nobody enumerated
// still gets gated. authoredFields stays as the named/typed view (better
// findings, and it carries from.member), but the structural sweep is what
// makes SECURITY 4's "every authored outbound field is filter input" literally
// true rather than "every field someone remembered to list".
function structuralFields(body, prefix, out, depth) {
  const acc = out || {};
  if (depth > 4 || body === null || body === undefined) return acc;
  if (typeof body === 'string') { acc[prefix || 'body'] = body; return acc; }
  if (Array.isArray(body)) {
    body.slice(0, 64).forEach((v, i) => structuralFields(v, (prefix || 'body') + '.' + i, acc, (depth || 0) + 1));
    return acc;
  }
  if (typeof body === 'object') {
    for (const [k, v] of Object.entries(body)) {
      structuralFields(v, prefix ? prefix + '.' + k : k, acc, (depth || 0) + 1);
    }
  }
  return acc;
}
...
function gate(type, body, from, filterOpts) {
  const named = authoredFields(type, body, from);
  const structural = structuralFields(body, '', {}, 0);
  return sendGate(Object.assign(structural, named), filterOpts);
}
