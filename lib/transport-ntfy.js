...
class NtfyTransport {
  ...
  async fetch(cursor, limit, opts) {
    ...
    for (const row of rows.slice(0, T.SYNC_CANDIDATE_WINDOW)) {
      ...
      seenHandles.add(row.id);
      candidates.push({
        order: order++, sender: env.from.member, type: env.type,
        handle: row.id, message_id: row.id, unix_ts: Number(row.time) || 0, envelope: env,
      });
    }
    // PROTOCOL §6.1: the reserved priority floor must be REAL, not nominal.
    // A client analog of the relay's second bounded read - scan the rows that
    // fell past the candidate window for priority types (warn.*/note.blocker)
    // so a recent blocker in a >200-message backlog is not buried.
    for (const row of rows.slice(T.SYNC_CANDIDATE_WINDOW)) {
      if (seenHandles.has(row.id)) continue;
      const env = this._decode(row, now, true);
      if (!env || !T.isPriorityType(env.type)) { order++; continue; }
      seenHandles.add(row.id);
      candidates.push({
        order: order++, sender: env.from.member, type: env.type,
        handle: row.id, message_id: row.id, unix_ts: Number(row.time) || 0, envelope: env,
