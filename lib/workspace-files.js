...
function writeWorkspacePublic(root, config, opts) {
  ...
  return { file: p.workspace, public: pub, refused };
}
...
const SHARD_KINDS = Object.freeze(['claim', 'change', 'release', 'done', 'parting']);
...
function appendShardRecord(root, record, opts) {
  ...
  for (const [k, v] of Object.entries(record.fields || {})) {
    ...
    if (Array.isArray(v)) {
      const list = v.slice(0, 64).map((x) => escape.escapeField(k === 'files' ? 'path' : k, x, { singleLine: true })).filter(Boolean);
...
function projectTasks(root, opts) {
  ...
  for (const r of all.slice().reverse()) {
    ...
    if (!key) continue;
    if (r.kind === 'claim' || r.kind === 'change') open.set(r.member + ' ' + key, { member: r.member, subject: r.fields.subject || key, at: r.at });
    if (r.kind === 'release' || r.kind === 'done' || r.kind === 'parting') open.delete(r.member + ' ' + key);
  }
...
module.exports = {
  ...
  shardFileName, shardPath, appendShardRecord, readShard, listShards, parseShard,
  checkShardAuthors, recordShardWarnings, recordMemberEmail, knownMemberEmails,
  projectTasks, renderTasks,
  writeClaudeMdBlock, readClaudeMdBlock,
};
