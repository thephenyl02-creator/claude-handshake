...
// Deliberately narrow and greppable, matching the formats SECURITY.md 3 froze.
const SECRET_SHAPES = [
...
function tokenInHistory(root, opts) {
  ...
  const needles = o.needles || ['hsk_', 'hsr_', 'hsi1_'];
  const hits = [];
  const checked = [];
  for (const needle of needles) {
    const r = git(root, ['log', '--all', '-S', needle, '--oneline', '--max-count=' + HISTORY_MAX_COUNT],
