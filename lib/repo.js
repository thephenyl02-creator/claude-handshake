...
function tokenInHistory(root, opts) {
  ...
  const needles = o.needles || [
    'hsk_[0-9a-fA-F]{16,}_[0-9a-fA-F]{4,}',
    'hsr_[0-9a-fA-F]{16,}_[0-9a-fA-F]{4,}',
    'hsm_[0-9a-fA-F]{8,}_[0-9a-fA-F]{16,}',
    'hsi1_[A-Za-z0-9_-]{40,}',
  ];
  const hits = [];
  const checked = [];
  for (const needle of needles) {
    const r = git(root, ['log', '--all', '-S', needle, '--pickaxe-regex', '--oneline', '--max-count=' + HISTORY_MAX_COUNT],
