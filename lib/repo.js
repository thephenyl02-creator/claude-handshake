...
// V2-PLAN section 2.5 / section 14 item 39: network git gets its own constant,
// distinct from GIT_TIMEOUT_MS (5,000 ms, right for `rev-parse` and wrong for
// `fetch`), and it is used on OFF-HOOK paths only - the monitor's own clock and
// the CLI. Every hook path threads a deadline and takes a slice of what is
// left. It lives here, beside the other two, because this is the module that
// owns what a git call costs; lib/state-branch.js re-exports it rather than
// keeping a second copy (V2-PLAN 10.1's Touches).
...
function defaultRunner(cmd, args, opts) {
  ...
  for (const bin of attempts) {
    const r = spawnSync(bin, args, {
      ...
      windowsHide: true,
      shell: false,
      // STDIN, and it is content and never argv. One caller needs it:
      // `git hash-object -w --stdin --path=<p>`, which hashes the bytes the
      // commit scanner actually looked at rather than re-reading the file a
      // second time [C lib/state-branch.js buildStateCommit]. Passing content on
      // a pipe is what keeps it out of the command line, where a length cap and
      // a quoting rule would both apply.
      input: typeof o.input === 'string' ? o.input : undefined,
      // THE HARDENING IS APPLIED LAST, AFTER the caller's env, and the order is
      // the property rather than a style choice. `GIT_TERMINAL_PROMPT=0` is
      // what makes a credential helper that would prompt FAIL instead of
      // hanging - V2-PLAN 10.1 states it as "a stated property rather than an
      // accident" - and merging the caller's env after the defaults turned it
      // into a default any caller could switch off. No caller does today; the
      // point is that none can.
      env: Object.assign({}, process.env, o.env || null,
...
function git(root, args, opts) {
  const o = opts || {};
  return (o.runner || defaultRunner)('git', args,
    { cwd: root, timeout: o.timeout || GIT_TIMEOUT_MS, env: o.env, input: o.input });
}
...
// A rev this module is willing to hand to git as a positional. The one caller
// that supplies one passes a fixed remote-tracking ref, but an option is a
// seam and a seam gets a validator: no leading `-` (an option-shaped rev is the
// one argv injection a no-shell runner still has), no `..`, no `@{`, no colon,
// no whitespace.
const REV_RE = /^[A-Za-z0-9][A-Za-z0-9._/-]{0,200}$/;

function safeRev(rev) {
  const s = rev === undefined || rev === null ? '' : String(rev);
  if (!REV_RE.test(s)) return null;
...
//
// `opts.rev` scopes the walk to a REF instead of `HEAD`'s history, which is
// V2-PLAN 10.1's Touches row for this function: on the state branch a peer's
// shard exists only on the fetched ref, so a HEAD-scoped `git log` answers
// `uncommitted` for every one of them. It is one argument on one command, not
// a second way to shell out - the previous build kept a near-copy of this
// function in lib/workspace-files.js, which is two spellings of one query.
//
// The COMMITTER email comes back beside the author's, because on the state
// branch the split (author = the member, committer = the tool) is the only
// locally derivable evidence that a shard on that branch was written by this
// tool at all (section 4.4 rule 3's derived peer line).
function lastCommitEmail(root, relPath, opts) {
  const o = opts || {};
  if (!root) return { ok: false, reason: 'not_a_repo', email: null, committer: null, sha: null };
  const args = ['log', '-1', '--format=%ae%x09%ce%x09%H'];
  if (o.rev !== undefined && o.rev !== null && o.rev !== '') {
    const rev = safeRev(o.rev);
    if (!rev) return { ok: false, reason: 'bad_ref', email: null, committer: null, sha: null };
    args.push(rev);
  }
  args.push('--', relPath);
  const r = git(root, args, o);
  if (!r.ok) {
    ...
    if (/does not have any commits|unknown revision|bad default revision|ambiguous argument .HEAD./i.test(r.stderr || '')) {
      return { ok: true, reason: 'uncommitted', email: null, committer: null, sha: null };
    }
    return { ok: false, reason: 'git_error', email: null, committer: null, sha: null };
  }
  const line = r.stdout.split(/\r?\n/)[0] || '';
  if (!line.trim()) return { ok: true, reason: 'uncommitted', email: null, committer: null, sha: null };
  const [email, committer, sha] = line.split('\t');
  return {
    ok: true, reason: null,
    email: (email || '').trim() || null,
    committer: (committer || '').trim() || null,
    sha: (sha || '').trim() || null,
  };
}
...
module.exports = {
  ...
  GIT_TIMEOUT_MS, HISTORY_TIMEOUT_MS, GIT_NETWORK_TIMEOUT_MS, safeRev,
